...
class NetworkTopology {
  ...
  createDevice(type, options = {}) {
    return new Promise((resolve) => {
      ...
      const finalizeDevice = (fabricObj) => {
        ...
        // 创建ID
        const deviceId = `device_${Date.now()}_${Math.floor(
          Math.random() * 1000
        )}`;
        ...
        // 触发事件
        this._triggerEvent("deviceCreated", {
          device: fabricObj,
          id: deviceId,
        });
      ...
      if (iconPath) {
        fabric.Image.fromURL(
          ...
          (img) => {
            // 调整尺寸
            const scale =
              (deviceOptions.size * 0.6) / Math.max(img.width, img.height);
            img.scale(scale);
  ...
  addConnection(source, target, type = "ethernet", networkInfo = null) {
    ...
    const connType =
      this.connectionTypes[type] || this.connectionTypes.ethernet;
    ...
    if (
      !networkInfo &&
      (source.deviceType === "firewall" || target.deviceType === "firewall")
    ) {
      // 确定哪个是防火墙
      ...
      if (deviceIP) {
        ...
        if (ipParts.length === 4) {
          ...
          if (deviceIP.startsWith("192.168.200.")) {
            ...
            // 对于其他网段，使用网关IP（通常是.1或.254）
            const ipParts = deviceIP.split(".");
            if (ipParts.length === 4) {
              firewallIP = `${ipParts[0]}.${ipParts[1]}.${ipParts[2]}.1`;
            }
          }
    ...
    // 如果有网络信息，添加网络标签（暂时禁用，使用手动创建的白色标签）
    // if (networkInfo) {
    ...
    // 触发连接创建事件
    this._triggerEvent("connectionCreated", {
      connection: line,
      source,
      target,
    });
  ...
  _addNetworkLabels(line, source, target, networkInfo) {
    ...
    // 计算连接线的角度
    const angle =
      (Math.atan2(target.top - source.top, target.left - source.left) * 180) /
      Math.PI;
  ...
  _getDefaultName(type) {
    ...
    const count =
      Object.values(this.devices).filter((d) => d.deviceType === type).length +
      1;
